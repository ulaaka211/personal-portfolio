      ...
      <div className="relative max-w-6xl mx-auto w-full px-5 md:px-10 py-20 grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
        ...
        <div className="flex flex-col gap-6 min-w-0">
          ...
          >
            ...
            <span className="text-[#00d4ff] font-mono">Invescore NBFI</span> —
            from building modular, high-performance React interfaces to
            designing ACID-compliant PostgreSQL pipelines for automated
            post-loan workflows.
          </motion.p>
          ...
          >
            {SKILLS.frontend.slice(0, 4).map((s) => (
              <NeonBadge key={s} color="cyan">
                {s}
              </NeonBadge>
            ))}
            {SKILLS.backend.slice(0, 3).map((s) => (
              <NeonBadge key={s} color="purple">
                {s}
              </NeonBadge>
            ))}
          </motion.div>
